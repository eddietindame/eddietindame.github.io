...
const Heart = ({ size, colour, accent }: HeartProps) => {
  ...
  return (
    <div className={[S['heart'], isActive ? S['heart--active'] : ''].join(' ')}>
      <svg
        className={S['heart__svg']}
        width={`${size}px`}
        height={`${size}px`}
        viewBox={`0 0 ${size * 2} ${size * 2}`}
      >
        <path
          className={S['heart__svg__path']}
      ...
      <div className={S['heart__bg']} />
      {/* @ts-expect-error - weird react-spring types */}
      <animated.div className={S['heart__message']} style={spring}>
        <div className={S['heart__message__inner']}>
